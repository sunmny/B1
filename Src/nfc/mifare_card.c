...
{	
	...
	send_buff[1]=block;//块地址
  	...
  	result =Pcd_Comm(Transceive,send_buff,2,rece_buff,&rece_bitlen);//
		//print_msg("Mifare_Blockwrite 1 = 0x%x\r\n",result);
		//print_msg("Mifare_Blockwrite 1 rece_buff = 0x%x\r\n",result);
		//{
		//  uint8_t i = 0;
		//	for(i = 0;i<rece_bitlen;i++)
		//	print_msg(" 0x%x",rece_buff[i]);
		//}
		//print_msg("\r\n");
	...
	//print_msg("Mifare_Blockwrite 2 = 0x%x\r\n",result);
	if ((result!=OK )|((rece_buff[0]&0x0F)!=0x0A)) //如果未接收到0x0A，表示无ACK
		return ERROR;
	return OK;
}
