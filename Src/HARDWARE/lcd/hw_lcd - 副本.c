...
volatile unsigned char lcd_gram[128][8];
...
void hw_lcd_power(GPIO_PinState state)
{	
  	//上电
...
while(1)
{
  ...
  //lcd_showstring(12,12,12,"123456789");
  //lcd_refresh();
	
//	delay_ms(500);
	
	lcd_showbattery(12,12, 5);
  lcd_refresh();
	delay_ms(500);
...
void lcd_refresh(void)
{
    ...
    for(i=0; i<8; i++)
    {
       ...
       // lcd_writedata(0);
			  ...
			  lcd_writecommand (0xb0+i);    //设置页地址（0~7）
        ...
        {
					  delay_ms(5);
            lcd_writedata(lcd_gram[128-j][8-i]);
        }
    }
}
...
//*p:字符串起始地址
//用16字体
void lcd_showstring(uint32 x,uint32 y,uint32 font,const uint8 *p)
{
    uint8 xsize,ysize;
...
{
    uint32 i, j, count, len, temp;
    const uint8 *p;

    switch(icon)
    {
    case ICON_SIGNAL_STRENGTH :
        p = SignalStrength_Font[level];
        len = 62;
        break;
    case ICON_NETWORK_CONNECTION :
        p = NetworkConnection_Font[level];
        len = 22;
        break;
    case ICON_BATTERY :
        p = Battery_Font[level];
        len =44;
        break;
		case ICON_ERWEIMA:
			  p = Battery_Font[0];
...
void lcd_showbattery(uint32 x,uint32 y, uint32 level)
{
    if(level <= 5)
        lcd_showicon(x, y, level, ICON_BATTERY, 1);
}

void lcd_shownetwork(uint32 x, uint32 y, uint32 status)
{
    if(status <= 3)
        lcd_showicon(x, y, status, ICON_NETWORK_CONNECTION, 1);
}

void lcd_showsignal(uint32 x, uint32 y, uint32 level)
{
    if(level <= 5)
        lcd_showicon(x, y, level, ICON_SIGNAL_STRENGTH, 1);
}
